...
\end{align*}
To simplify calculations, we will focus on power spectral density (PSD) defined as $P(f)$ in dBm/Hz, ignoring the receiver bandwidth for now. Additionally, it is safe to assume that $P_R = P_{noise}$ when the antenna is pointed at a cold sky.
