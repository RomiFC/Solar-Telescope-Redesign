...
\subsection{Architecture \color{red}(INCOMPLETE)}
\begin{figure}[!ht]
    \begin{center}
        \begin{circuitikz}
            \draw(0, 0)
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            legend style = {
                ...
                \addplot[opacity=0.2, green] fill between [of=f and xaxis, soft clip={domain=-47.5:-32.5}];
                \draw[dashed] (axis cs:-47.5,0) -- (-47.5, 1.25);
                \draw[dashed] (axis cs:-32.5,0) -- (-32.5, 1.25);
                \draw (axis cs:-40,1.25)
                node[anchor=south]{Ideal Range};

                \addplot[opacity=0.2, red] fill between [of=f and xaxis, soft clip={domain=-25:-10}];
                \draw[dashed] (axis cs:-25,0) -- (-25, 2.25);
                \draw[dashed] (axis cs:-10,0) -- (-10, 2.25);
                \draw (axis cs:-17.5,2.25)
...
\begin{tikzpicture}
    \begin{axis}[
        title={Spectrum Analyzer Plot, $P_{\mu}$= -92.6 dBm},
        xlabel=Frequency (MHz),
        ylabel={Power at 100 kHz RBW(dBm)},
        grid=major,
        xmin=8350, xmax=8450,
        ymin=-110, ymax=-85,
        xtick={8375, 8400, ..., 8450},
        ytick={-110, -105, ..., -85},
        legend style = {
            align = left,
            cells = {anchor=east},
        ...
        \addplot[smooth, blue!70] table [x index=0,y index=1] {data/spectrumTrace.txt};
        \addlegendentry{Signal Power}
        \addplot[smooth, red!80] table [x index=0,y index=2] {data/spectrumTrace.txt};
        \addlegendentry{Noise Power}
    \end{axis}
\end{tikzpicture}
...
\begin{align*}
    P_R &= P_{\mu} - G_{dB} - \left(RBW\right)_{dB} + \left(BW\right)_{dB}\\
        ...
        &= -82.2\ \text{dBm}
\end{align*}
This measurement is about 5 dB greater than calculated but is within the degree of accuracy necessary for this design. It's possible this error was caused by assuming equal power throughout the entire passband, or due to the fact that this operating frequency is outside the bandwidth of the amplifiers used, thus resulting in nonlinear gain across frequency. Note that the noise power in Fig.~\ref{fig:specan1} was measured with a 50$\Omega$ load at the spectrum analyzer input and no amplifiers.

In another experiment on a different day, traces were taken with the telescope pointed at both the sun and ground in a cloudy and sunny condition. This time with 42 dB of gain from a ZX60-83LN-S+ and an ALS-04-0149 in that order, along with the 7.8 -- 8.9 GHz bandpass filter between them.
\begin{figure}[H]
    \begin {center}
        \begin{tikzpicture}
            \begin{axis}[
                ...
                \addplot[smooth, blue!70] table [x = FREQ-B, y = TRACE-B] {data/SpectrumRHS.txt};
                \addlegendentry{Telescope at Ground}
                \addplot[smooth, red!80] table [x = FREQ-A, y = TRACE-A] {data/SpectrumRHS.txt};
                \addlegendentry{Telescope at Sky}
            \end{axis}
        \end{tikzpicture}
        \caption{Signal trace with 42 dB of gain and a 7.8 -- 8.9 GHz filter.}\label{fig:specan2}
    \end {center}
\end{figure}
Figure~\ref{fig:specan2} shows a roughly 2 dB drop in power when the telescope moved from the ground towards the horizon/sky; this power reading also did not change when pointing directly at the sun versus anywhere else in the sky. Additionally, the right and left-hand side ports on the antenna feed did not make a difference to the readings. The ripples in the trace are caused by the filter response and the downward slope is due to reaching the end of each LNA's operating frequency.

Power calculations conducted in the same manner as the trace in Fig.~\ref{fig:specan1} show a channel power of about -80 dB in the passband.

\appendix
\section{S-Parameters \color{red}TEMPORARY}
\begin{figure}[H]
    \begin {center}
        \begin{tikzpicture}
            \begin{axis}[
                ...
                \addplot[smooth, red!80] table [x index=0, y index=1] {data/Amplifiers and Filter Sparam.txt};
                \addlegendentry{ZX60-83LN-S+}
                \addplot[smooth, blue!70] table [x index= 0, y index=2] {data/Amplifiers and Filter Sparam.txt};
                \addlegendentry{ALS-04-0149}
            \end{axis}
        \end{tikzpicture}
        \caption{LNA S-parameters. At 8.4 GHz, gain is about 15 dB for the ZX60 and 27 dB for the ALS.}\label{fig:sparam1}
    \end {center}
\end{figure}
\begin{figure}[H]
    \begin {center}
        \begin{tikzpicture}
            \begin{axis}[
                ...
                \addplot[smooth, red!80] table [x index=0, y index=3] {data/Amplifiers and Filter Sparam.txt};
            \end{axis}
        \end{tikzpicture}
        \caption{Unmarked bandpass filter S-parameters.}\label{fig:sparam2}
    \end {center}
\end{figure}
