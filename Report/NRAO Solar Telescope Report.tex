...
\usepackage{circuitikz} % Used to draw circuit and block diagrams
\usepackage{appendix}   
...
\begin{circuitikz} \draw
    (0, 0) node[rxantenna, xscale=-1, label = X-band feed]{}
    to[amp = LNA]++(4, 0)
    to[bandpass = 7.8--8.9 GHz]++(1,0)
    to[empty Schottky diode = Rectifier]++(3,0)
    to[capacitor]++(0, -2)
    (8,-2) node[tlground]{};
    \draw (8, 0)
    to[short = DC Out, -o]++(1,0);
\end{circuitikz}
...
In order to confirm these calulations, the antenna was connected to a 
