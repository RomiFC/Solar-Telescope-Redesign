...
\begin{figure}[!ht]
    \begin{center}
        \begin{circuitikz}
            ...
            to[amp = RF Gain Block, boxed] ++ (2.25, 0)
            to[bandpass = Filter] ++ (2.25,0)
            to[detector = Rectifier] ++ (2.25,0)
            to[amp = DC Gain Block, boxed] ++ (2.25,0)
            -- ++ (1, 0)
            % Bottom block
            |- ++ (-1, -3)
            to[R, name=r1, l_=R1] ++ (-3, 0)
            coordinate (branch1)
            |- ++ (-1, 1)
            to[qvprobe = Voltmeter, mirror, name=vm] ++ (-1 ,0)
            -| ++ (-1, -1)
            (branch1)
            |- ++ (0, -1)
            to[R, name=r2, l_=R2] ++ (-3 ,0)
            -| ++ (0, 1)
            coordinate (branch2)
            -| ++ (-2, -0.5)
            node[sground]{};
            % Rectangle
            \draw (branch2) ++ (-0.5, 0)
            coordinate (boxedgeleft);
            \draw (r1.center) ++ (1, 0)
            coordinate (boxedgeright);
            \node[rectangle, draw, dashed, fit=(boxedgeleft) (r2) (vm) (boxedgeright),]{};
            % Rectangle Caption
            \draw (boxedgeleft) ++ (-0.25, 0.5)
            [<-, dashed] -- ++ (165:2)
            node[anchor=south]{Output scaling circuit};
        \end{circuitikz}
...
The output scaling circuit is a simple voltage divider that ensures the maximum and minimum voltages read by the voltmeter are within its range. Even with no RF input, the quiescent output voltage of the ADL5902 is around 400 mV; with any significant amplification this can easily reach the 3 volt maximum of the meter. In essence, the DC gain block adjust the slope of the $P_{in}$ vs. $V_{out}$ curve, whereas the output scaling circuit shifts the linear region curve down.
